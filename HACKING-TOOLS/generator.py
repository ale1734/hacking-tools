...
var = input(f"{r}[{w}-{r}]{y}Palabras a manejar -> {g}{b}").lower()
var = var + var.upper()
longitud = input(f"{r}[{w}-{r}]{y}Longitud -> {g}{b}")
...
print(f'{r}[{w}-{r}]{y}Palabras a manejar -> {g}{dic}{b}')
print (f'{r}[{w}-{r}]{y}Se generara un archivo llamado > {r}{nombre} {y}< que contendra el diccionario{b}')
numero_de_claves = len(dic)**int(longitud)
B = numero_de_claves*int(longitud)+numero_de_claves
MB = B/1048576
print(f'{r}[{w}-{r}]{y}El numero de palabras generadas sera -> {numero_de_claves}{b}')
print(f'{r}[{w}-{r}]{y}El archivo pesara {B} B /{B/1024} kB /{MB} MB /{MB/1024} GB' )
continuar = input(f"{g}Desea continuar? s/n -> {b}")
print(f"{r}[{w}-{r}]{y}Generando diccionario...")

if continuar != 's':
	print(f'{g}{r}[{w}-{r}]{y}adios...')
	exit()
